...
It owns the hypervisor, the vsock transport and the sandboxes independent of any
...
The machine running the sandboxes is a hub. Clients attach to it and authenticate
per connection by key; the hub owns the sandboxes and any resident credentials.
The hub is whichever machine runs the sandboxes — a server, a VPS, or a personal
...
- there is no ambient user agent or keychain to lean on, so credentials that
  must stay out of a sandbox are held on the hub and proxied (see
...
- there is no ambient user checkout, so a working tree is never a repository
  source; getting code into a sandbox is payload, not enclosure (see
  `sandbox-only-scope.md`)

## consequences

- a sandbox belongs to whoever holds an authorized key, not to a system account
- the same model serves humans, CI and machines without any of them existing
  as a linux user on the hub
- host root still reaches every sandbox through the hypervisor regardless of keys;
