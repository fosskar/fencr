...
- `outbound` defaults to `[ "internet" ]`; setting it replaces that rather
  ...
  address, never on loopback and never on the vm's own subnet: the egress
  ...
  from the `blocked` list `egressConfig` renders. The vm's nftables filter chains run at `filter - 1`, before
