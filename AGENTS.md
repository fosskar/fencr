...
and `fencr checkpoint`/`fencr restore` change a sandbox's disk state. Instance and
...
guidance changed; `checkpoints.md` holds the copy model and why the sandbox is not
...
- `pkgs/cli/cli.go` is the fencr command, one Go program with the standard
  ...
  text the firewall writes. `fencr status` also reads `GET /` from each sandbox's
  Firecracker API socket with a two-second timeout. `VMM:` is separate from
  systemd state; an unavailable API does not mean the sandbox is stopped.
- `pkgs/egress/` is the sandbox's road out, one Go program, standard library only,
...
- Each sandbox runs as `fencr-<name>` with persistent state at
  `/var/lib/fencr-sandboxes/<name>/state.img`, mounted as the guest's root
...
- `outbound` defaults to `[ "internet" ]`; setting it replaces that rather
  ...
  but still blocks other special-use ranges. DNS means the sandbox's own egress
  ...
  address, never on loopback and never on the sandbox's own subnet: the egress
...
- `credentials` intercepts TLS for the credential's domain only: the guest's
  ...
  granted domain from the sandbox's own authority, which `fencr-<name>-ca.service`
  keeps in `/var/lib/fencr/ca/<name>`, so a certificate minted for one sandbox is
  ...
  the sandbox's egress unit requires before it starts; `secretSourceOf` picks
...
- Beyond `cpuQuota` and `memoryMax`: `diskBandwidth` and `networkBandwidth`
  ...
  default; `maxConnections` is a `ct count` rule in the sandbox's forward and input
...
- `treefmt.nix` enables nixfmt, deadnix, statix, mdformat and gofmt. The dev
  ...
  the CLI is installed by the NixOS module when sandboxes are declared.
...
- `checks/nixos-boot.nix` runs a Firecracker guest inside a NixOS test sandbox,
  requiring nested KVM; on x86_64 the test sandbox uses `-cpu host` because Firecracker
