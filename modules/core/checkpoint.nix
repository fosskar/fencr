...
in
{
  ...
  # the text and the derivation are separate so checks/core.nix can read the
  # script without import-from-derivation, which would need a builder of the
  # instance's own system
  checkpointScript =
    pkgs: instance:
    pkgs.writeShellScript "fencr-${instance.name}-checkpoint" (checkpointText pkgs instance);
